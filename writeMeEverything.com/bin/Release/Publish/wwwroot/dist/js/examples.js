$(function () {

    /**
     * Some examples of how to use features.
     *
     **/

    var SohoExamle = {
        Message: {
            add: function (message, type) {
                var chat_body = $('.layout .content .chat .chat-body');
                if (chat_body.length > 0) {

                    type = type ;
                    message = message;
                    var time = (new Date()).toISOString().replace(/T/, ' ').replace(/\..+/, '');
                    $('.layout .content .chat .chat-body .messages').append('<div class="message-item ' + type + '"><div class="message-content">' + message + '</div><div class="message-action">'+time + (type ? '<i class="ti-check"></i>' : '') + '</div></div>');

                    chat_body.scrollTop(chat_body.get(0).scrollHeight, -1).niceScroll({
                        cursorcolor: 'rgba(66, 66, 66, 0.20)',
                        cursorwidth: "4px",
                        cursorborder: '0px'
                    }).resize();
                }
            }
        }
    };



    setTimeout(function () {
        // $('#disconnected').modal('show');
        $('#call').modal('show');
    }, 2000);

    $(document).on('submit', '.layout .content .chat .chat-footer form', function (e) {
        e.preventDefault();

        var input = $(this).find('input[type=text]');
      

        var Id = input.attr("id");

  
        var message = input.val();



        message = $.trim(message);  

        if (message) {
            SohoExamle.Message.add(message, 'outgoing-message');


            $.ajax({
                url: "/messenger/addmessage?Id=" + Id + "&Message="+message,
                type: "get",
                dataType: "html",
                success: function (response) {
                    console.log(response);

                },
                error: function (error) {
                    console.log(error);
                }
            });
            input.val('');
        } else {
            input.focus();
        }
    });

    $(document).on('click', '.layout .content .sidebar-group .sidebar .list-group-item', function () {
        if (jQuery.browser.mobile) {
            $(this).closest('.sidebar-group').removeClass('mobile-open');
        }
    });


    $(document).on('click', '.addFriends .modal-dialog .modal-content .modal-footer button', function (e) {
        e.preventDefault();
        var email = $(".addfrendemail").val();
        var message = $(".addfrendmessage").val();

        $.ajax({
            url: "/messenger/addfriend?Email=" + email + "&Message=" + message,
            type: "get",
            dataType: "html",
            
            error: function (error) {
                console.log(error);
            }
        });

        $(".close").click();

        
        setTimeout(checkCookie, 1000);

    });




    function checkCookie() {
        if (typeof $.cookie('message') !== 'undefined') {

            var mes = $.cookie("message");
            $.removeCookie('message', { path: '/' });
            $.cookie("message", "", { expires: -1 });
            $.notify({
                // options
                message: mes
            }, {
                    // settings
                    type: 'info'
                }
            );
        }
    }
});